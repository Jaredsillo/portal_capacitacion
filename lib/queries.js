  ...
  return q(
    `SELECT s.id, s.nombre, s.descripcion, s.url_destino,
            ...
            CASE
              ...
              WHEN LOWER(m.archivo_path) REGEXP '\\.(ppt|pptx)$' THEN 'ppt'
...
// ── v2: Reporte de avance (para exportar / panel de RH) ──
export async function reporteAvance() {
  return q(
    `SELECT u.nombre_completo, u.correo, u.area, s.nombre AS sistema,
            CASE WHEN m.id IS NULL THEN 'Sin manual'
                 WHEN p.leido=1 THEN 'Leído' ELSE 'Pendiente' END AS estado_manual,
            p.fecha_leido
     FROM asignaciones asig
...
// ── v5: Panel de Talento Humano (solo lectura) ──
// Avance de cada colaborador activo: cuántos manuales le tocan y cuántos ya leyó,
// más el título de los que le faltan.
export async function resumenTH() {
  return q(
    `SELECT u.id, u.nombre_completo, u.correo, u.puesto, u.area, u.ultimo_acceso,
            COUNT(DISTINCT m.id) AS manuales_asignados,
            COUNT(DISTINCT CASE WHEN p.leido=1 THEN m.id END) AS manuales_leidos,
            GROUP_CONCAT(DISTINCT CASE WHEN m.id IS NOT NULL AND (p.leido IS NULL OR p.leido=0)
                         THEN COALESCE(m.codigo, m.titulo) END SEPARATOR ' · ') AS pendientes
     FROM usuarios u
     LEFT JOIN asignaciones asig ON asig.usuario_id = u.id
     LEFT JOIN sistemas s ON s.id = asig.sistema_id
     LEFT JOIN manuales m ON m.sistema_id = s.id
     LEFT JOIN progreso_usuario p ON p.manual_id = m.id AND p.usuario_id = u.id
     WHERE u.estado = 'alta'
     GROUP BY u.id
