...
// ── v2: Juego posterior al manual (sopa de letras / ahorcado) ──
// El campo "modelo" de quizzes guarda el tipo de juego elegido ("sopa" o "ahorcado").
export async function guardarQuiz(manualId, juego) {
  await q("UPDATE quizzes SET activo=0 WHERE manual_id=?", [manualId]);
  const r = await q("INSERT INTO quizzes (manual_id, modelo, activo) VALUES (?,?,1)",
    [manualId, juego.tipo_juego || "sopa"]);
...
export async function getQuizParaUsuario(sistemaId) {
  ...
  return {
    quizId: quizzes[0].id,
    tipoJuego: quizzes[0].modelo === "ahorcado" ? "ahorcado" : "sopa",
