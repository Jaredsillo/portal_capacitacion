...
## v3 · Evaluaciones capturadas por el administrador (sin IA)

Esta versión **no usa ninguna API de pago**. Las evaluaciones las crea el administrador a mano.

- **Evaluación por manual**: el admin escribe preguntas de **opción múltiple** y **verdadero/falso**,
  marca la respuesta correcta y (opcional) una explicación. Es **obligatoria**: el empleado debe
  aprobar (puntaje mínimo configurable, por defecto 60%) para completar el manual.
- **Sopa de letras**: el admin captura a mano las palabras clave (3 a 12 letras, sin acentos).
- **Aviso al admin**: cuando un empleado aprueba, se marca el manual como leído y se envía el correo.

### Cómo crear una evaluación (admin)
1. Pestaña **Sistemas** → crea el sistema si no existe.
2. Pestaña **Manuales y evaluaciones** → sube el PDF.
3. En la lista del manual, botón **Crear evaluación** → escribe las preguntas y las palabras → **Guardar**.
...
### Tablas usadas
`quizzes`, `preguntas`, `palabras_clave`, `intentos_quiz` (ya incluidas en `db/schema.sql`).
