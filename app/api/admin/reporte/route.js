...
export async function GET() {
  ...
  const head = ["Nombre", "Correo", "Área", "Sistema", "Manual", "Fecha lectura", "Mejor puntaje"];
  const esc = (v) => `"${String(v ?? "").replace(/"/g, '""')}"`;
  const csv = [head.join(",")].concat(
    filas.map((f) => [f.nombre_completo, f.correo, f.area, f.sistema, f.estado_manual,
      f.fecha_leido ? new Date(f.fecha_leido).toLocaleString("es-MX") : "",
      f.mejor_puntaje != null ? f.mejor_puntaje + "%" : ""].map(esc).join(","))
  ).join("\n");
